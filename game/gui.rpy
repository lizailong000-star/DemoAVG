...
################################################################################
## v2.2t patch: SDK 默认 screens.rpyb / quick_menu / history 依赖兜底
##
## 项目从未引入 SDK 默认 screens.rpy 源码，DemoAVG/game 下没有 screens.rpy / options.rpy；
## 所有 Ren'Py 原生 UI 走 cache/screens.rpyb 编译产物运行。这些产物里有时会引用
## gui.scale / gui.nvl_list_length 等"DemoAVG gui.rpy 没定义"的属性，导致 quick_menu /
## history 抛 StoreModule has no attribute 'scale' / 'nvl_list_length'。
##
## 这里在 gui.rpy 末尾做兜底：如果不存在就补一个安全默认值。
## 不覆盖任何已有定义。
################################################################################

init -1 python:
    if not hasattr(gui, "scale"):
        gui.scale = 1.0
    if not hasattr(gui, "nvl_list_length"):
        gui.nvl_list_length = 100